...
router.get('/fetch-monthly-report', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const brand = await Brand.findById(brandObjId);
        if(!brand){
            return res.status(400).json({ error : "Brand not found" })
        }
        // Step 2: Fetch Project Data
        const project = await Projects.find({ project_id: projectObjId, brand_id: brandObjId});
        if (!project) {
            return res.status(404).json({ error: 'Project not found' });
        }
        // Step 3: Fetch Lead Details
        const lead = await User.findById(
            project.lead_id,
