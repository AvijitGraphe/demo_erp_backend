...
router.get('/fetchalltickets', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const filters = {};
        if (subject) {
            filters.subject = { $regex: subject, $options: 'i' }; // Search by partial match (case-insensitive)
        }
        ...
        // Fetch all tickets with applied filters and associated data
        const tickets = await RaiseTicket.aggregate([
            {
              $match: filters, // Match the filters to get relevant tickets
            },
            {
              $lookup: {
                from: 'users', // Assuming `users` collection is named "users"
                localField: 'raiserId', // Field in RaiseTicket collection
                foreignField: '_id', // Field in Users collection
                as: 'raiser',
              },
            },
            {
              $unwind: { path: '$raiser', preserveNullAndEmptyArrays: true }, // Unwind the raiser array
            },
            {
              $lookup: {
                from: 'profileimages', // Assuming `profileimages` collection is named "profileimages"
                localField: 'raiser.profileImageId', // Field in Users collection referencing profile image
                foreignField: '_id',
            ...
            {
              $lookup: {
                from: 'userdetails', // Assuming `userdetails` collection is named "userdetails"
                localField: 'raiser.userDetailsId', // Field in Users collection referencing user details
                foreignField: '_id',
            ...
            },
            {
              $lookup: {
                from: 'users',
                localField: 'resolverId', // Assuming there's a resolverId field in RaiseTicket
                foreignField: '_id',
                as: 'resolver',
              },
            },
            ...
            {
              $lookup: {
                from: 'ticketimages', // Assuming `ticketimages` collection is named "ticketimages"
                localField: '_id', // Reference to the ticket
                foreignField: 'ticketId', // Assuming there is a `ticketId` field in `ticketimages` collection
                as: 'images',
              },
            },
            {
              $addFields: {
                imageCount: { $size: '$images' }, // Add an aggregated field to count the images
              },
            },
            {
              $project: {
                ...
                'images': 0, // Optionally remove images if you just want the count
              },
            },
            {
              $group: {
                _id: '$_id', // Group by ticket ID
                status: { $first: '$status' }, // Example aggregation: keep status from the first document
        ...
        // Fetch counts of tickets by status using aggregation
        const statusCounts = await RaiseTicket.aggregate([
            {
                $group: {
                    _id: '$status',
                    count: { $sum: 1 }
